...
def place_outside_letters_to_middle(letter, row, col):
    if row == 0 and 1 <= col <= 4: #downward
        row += 1
        while grid[row][col] == 'X':
            row += 1
        grid[row][col] = letter
    elif row == 5 and 1 <= col <= 4: # upward
        row -= 1
        while grid[row][col] == 'X':
            row -= 1
        grid[row][col] = letter
    elif col == 0 and 1 <= row <= 4: # rightward
        col += 1
        while grid[row][col] == 'X':
            col += 1
        grid[row][col] = letter
    elif col == 5 and 1 <= row <= 4: # leftward
        col -= 1
        while grid[row][col] == 'X':
            col -= 1
        grid[row][col] = letter
...
for T in range(1, TC + 1):
    rows = [defaultdict(int) for _ in range(6)]
    cols = [defaultdict(int) for _ in range(6)]
    input_data = input().split()
    blocked_cells = list(map(int, input_data[:4]))
    letters_num = int(input_data[4])
    letters_and_pos = input_data[5:]
    ...
    middle_grid = return_middle_letters()

    print(f"#{T} {'.'.join(cell if cell is not None else '_' for cell in middle_grid)}")
